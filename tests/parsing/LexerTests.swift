...
class LexerTests: Test {
    ...
    static func testBasic() throws {
        ...
        try test.assert(lexer.next() == Token(type: atpkg.TokenType.Identifier, value: "source", line: 12, column: 8))
        try test.assert(lexer.next() == Token(type: atpkg.TokenType.OpenBracket, value: "[", line: 12, column: 15))
        try test.assert(lexer.next() == Token(type: atpkg.TokenType.StringLiteral, value: "src/**.swift", line: 12, column: 16))
        try test.assert(lexer.next() == Token(type: atpkg.TokenType.StringLiteral, value: "lib/**.swift", line: 12, column: 31))
        try test.assert(lexer.next() == Token(type: atpkg.TokenType.CloseBracket, value: "]", line: 12, column: 45))
        try test.assert(lexer.next() == Token(type: atpkg.TokenType.Terminal, value: "\n", line: 12, column: 46))
